...
def abrir_imagenes_en_carpeta(ruta_carpeta):
    ...
    Args:
        ruta_carpeta (str): Ruta de la carpeta que contiene las imágenes.

    Returns:
        list: Lista de tuplas (nombre_archivo, imagen).
    """
    imagenes = []
    try:
        for archivo in os.listdir(ruta_carpeta):
            ...
            if os.path.isfile(ruta_completa) and archivo.lower().endswith(('.jpg', '.png', '.jpeg', '.bmp', '.tiff')):
                ...
                if imagen is not None:
                    imagenes.append((archivo, imagen))  # Asegúrate de que aquí agregamos una tupla
                else:
...
def agregar_texto_con_contorno(imagen, coordenadas_metros, fecha, fuente_path, tamano_fuente=1, color_texto=(255, 255, 255), color_contorno=(0, 0, 0), desplazamiento_contorno=2):
    """
    ...
    try:
        ...
        x = ancho_imagen - 750  # Margen derecho
        y = altura_imagen - 10  # Margen inferior
